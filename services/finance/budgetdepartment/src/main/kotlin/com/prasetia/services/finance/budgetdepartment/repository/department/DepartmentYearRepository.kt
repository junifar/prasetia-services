...
import com.prasetia.services.finance.budgetdepartment.model.department.DepartmentYear
    ...
    companion object {
        const val QUERY = """
                            ...
                            LEFT JOIN (
                                                ...
                                                WHERE
                                                "public".project_project.site_type_id IN (36, 37, 38, 40, 41, 42, 43, 45, 46, 47, 48, 61, 64, 65, 68, 70)
                                                GROUP BY
                                                "public".budget_plan."id"
                                                ) AS "c" ON "public".budget_plan."id" = C.id
                            WHERE
                            "public".budget_plan.site_type_id IN (36, 37, 38, 40, 41, 42, 43, 45, 46, 47, 48, 61, 64, 65, 68, 70) AND
                            "public".budget_plan."state" NOT IN ('draft', 'cancel', 'approve1') AND
                            "public".hr_department."name" IS NOT NULL AND
