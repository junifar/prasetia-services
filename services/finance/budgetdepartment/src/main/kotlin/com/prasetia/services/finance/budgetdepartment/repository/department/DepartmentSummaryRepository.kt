...
import com.prasetia.services.finance.budgetdepartment.model.department.DepartmentSummary
...
@Repository
interface DepartmentSummaryRepository:CrudRepository<DepartmentSummary, Long>{
    companion object {
        const val QUERY = """
                            SELECT
                            ROW_NUMBER() OVER (ORDER BY EXTRACT(YEAR FROM "public".budget_plan.periode_start)) AS id,
                            EXTRACT(YEAR from "public".budget_plan.periode_start) AS periode,
    ...
    fun getDepartmentSummary():Iterable<DepartmentSummary>
}
